...
document.getElementById('swapCoverBack').addEventListener('click', () => {
    const coverIndex = pageOrder.indexOf('cover');
    const backIndex = pageOrder.indexOf('back');
    [pageOrder[coverIndex], pageOrder[backIndex]] = [pageOrder[backIndex], pageOrder[coverIndex]];
    document.getElementById('previewBookletBtn').click();
});

document.getElementById('swapContent').addEventListener('click', () => {
    const content1Index = pageOrder.indexOf('content1');
    const content2Index = pageOrder.indexOf('content2');
    [pageOrder[content1Index], pageOrder[content2Index]] = [pageOrder[content2Index], pageOrder[content1Index]];
    document.getElementById('previewBookletBtn').click();
});
...
document.getElementById('createBookletBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        const newPage1 = bookletPdf.addPage([page0Dims.width * 2, page0Dims.height]);
        newPage1.drawPage(embeddedPages[0], { x: 0, y: 0 });
        newPage1.drawPage(embeddedPages[1], { x: page0Dims.width, y: 0 });
        
        // Tạo trang thứ hai: 2 trang cuối trong pageOrder (side by side)
        const page2Dims = embeddedPages[2].scale(1);
        const newPage2 = bookletPdf.addPage([page2Dims.width * 2, page2Dims.height]);
        newPage2.drawPage(embeddedPages[2], { x: 0, y: 0 });
        newPage2.drawPage(embeddedPages[3], { x: page2Dims.width, y: 0 });
        
        const pdfBytes = await bookletPdf.save();
        downloadPDF(pdfBytes, 'booklet.pdf');
        showOutput('Tạo sổ PDF thành công!');
    } catch (error) {
        showOutput('Lỗi: ' + error.message, true);
    }
});
